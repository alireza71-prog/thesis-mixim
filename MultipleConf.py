...
def main(rate):
    ...
    n_cascade = 2
    #Dummies
    link_dummies = False #config['DUMMIES']['link_based_dummies']
    try:
    ...
    # weights = []
    # weight_l1 = [1/6,1/6,1/6,1/6,1/6,1/6]
    # weight_l2 = [1/3,1/3,1/3, 0, 0 ,0]
    # weight_l3 = [1/3,1/3,1/3, 0, 0 ,0]
    # weights.append(weight_l1)
    # weights.append(weight_l2)
    # weights.append(weight_l3)
    # print("weights", weights)

    simulation = Simulation(mix_type=mix_type, simDuration=50, rate_client=1/lambda_c, mu=mu, logging=True,
                            topology=topology,fully_connected= fully_connected, n_clients=n_clients, flushPercent=pool_size, printing=True, flushtime=timeout, capacity=capacitiesOrganized, bandwidth=threshold, routing=routing, n_layers=n_layer,
                            n_mixes_per_layer=n_mix_per_layer,corrupt= corrupt_mixes,UniformCorruption= balanced_corruption,probabilityMixes=weights,nbr_cascacdes = n_cascade, ClientDummy=None, LinkDummies = link_dummies,RateDummies = 1,
